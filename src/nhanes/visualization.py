...
# ---------------- Fig 2 (PPT): F1 vs EOpp trade-off (RQ1) ------------
def ppt_fig2_nhanes_tradeoff(cfg, attrs=None, out="ppt_fig2_nhanes_tradeoff"):
    """PPT-styled copy of paper_fig2_nhanes_tradeoff (larger fonts, thicker
    marks). One panel per attribute, scatter F1 vs EOpp, degenerate x-marked."""
    global FIG
    FIG = cfg.FIG
    ppt_style()
    df = cfg.mit_df
    ...
    for ax, attr, title in zip(axes, attrs, titles):
        ...
        for _, r in sub.iterrows():
            ...
            ax.scatter(r["f1"], r[EOPP_PAPER],
                       c="none" if degen else col,
                       edgecolors=col if degen else "white",
                       marker=_MODEL_MARKERS_PAPER[r["model"]], s=122.88,
                       linewidth=1.76 if degen else 0.64, alpha=0.9, zorder=3)
            if degen:
                ax.scatter(r["f1"], r[EOPP_PAPER], marker="x", c="red",
    ...
    mh = [Line2D([0], [0], marker="o", color="w",
                 ...
                 label=METHOD_SHORT[m]) for m in METHOD_ORDER]
    kh = [Line2D([0], [0], marker=_MODEL_MARKERS_PAPER[m], color="w",
                 markerfacecolor="gray", markersize=10.4,
                 label=MODEL_SHORT[m]) for m in MODEL_ORDER]
    kh += [Line2D([0], [0], marker="x", color="red", lw=0,
...
# ------------- Fig 3 (PPT): method comparison per classifier (RQ2) ---
def ppt_fig3_adult_method_gaps(cfg, attrs=None, out="ppt_fig3_adult_method_gaps"):
    """PPT-styled copy of paper_fig3_adult_method_gaps (larger fonts, thicker
    marks). Gray baseline level + per-method outcome per classifier."""
    global FIG
    FIG = cfg.FIG
    ppt_style()
    df = cfg.mit_df
    ...
    for ax, attr, title in zip(axes, attrs, titles):
        ...
        for i, m in enumerate(MODEL_ORDER):
            ...
            for j, meth in enumerate(METHOD_MARKERS):
                ...
                ax.scatter(xo, v, marker=METHOD_MARKERS[meth],
                           color=METHOD_COLORS[meth], s=40.96, zorder=4)
        ax.axhline(0.1, color="red", ls="--", lw=1.12, alpha=0.6)
        ax.set_xticks(range(len(MODEL_ORDER)))
        ax.set_xticklabels([MODEL_SHORT[m] for m in MODEL_ORDER])
        ax.set_title(title, pad=10)
...
# ------------ Fig 4 (PPT): amplification, compact single-column (RQ3) -
def ppt_fig4_nhanes_amplification(cfg, attrs=None, out="ppt_fig4_nhanes_amplification"):
    """PPT-styled copy of paper_fig4_nhanes_amplification (larger fonts, thicker
    marks). Amplification per method and classifier."""
    global FIG
    FIG = cfg.FIG
    ppt_style()
    path = _diag_csv(cfg, "manufactured_disparity")
    if not path.exists():
        print(f">>> skip d2_ppt: missing {path.name}")
        return
    df = pd.read_csv(path)
    attrs = attrs or [a for a in cfg.ATTR if a in df["sensitive_attr"].unique()]
    titles = [cfg.ATTR_TITLES.get(a, a) for a in attrs]
...
# ---- Fig 5 (PPT): gender baseline vs best mitigation, cross-dataset -
def ppt_fig5_gender_cross(datasets, cfg_for_fig, out="ppt_fig5_gender_cross",
                          legend_on=None):
    """PPT-styled copy of paper_fig5_gender_cross (larger fonts, thicker marks).
    Single row, gender-only baseline vs best post-mitigation EOpp across
    datasets."""
    global FIG
    FIG = cfg_for_fig.FIG
    ...
    items = [(n, p, a) for n, (p, a) in datasets.items()
             if Path(p).exists()]
    if not items:
        print(">>> skip x3_ppt: no dataset csvs found")
        return
    n = len(items)
